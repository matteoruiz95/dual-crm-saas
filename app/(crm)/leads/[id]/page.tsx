    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
        <div>
          ...
          <p className="mt-1 text-slate-500">{lead.company || "Sin empresa"} · {lead.position || "Sin cargo"}</p>
        </div>
        <form action={changeStatus} className="flex gap-2">
          <Select name="status" defaultValue={lead.status}>
            {LEAD_STATUS.map((status) => <option key={status.value} value={status.value}>{status.label}</option>)}
          </Select>
      ...
      <div className="grid gap-6 xl:grid-cols-[1.5fr_1fr]">
        <Card>
          ...
          <div className="mt-4 grid gap-4 md:grid-cols-2">
            ...
            <Info label="Estado" value={LEAD_STATUS.find((item) => item.value === lead.status)?.label ?? lead.status} />
            <Info label="Valor estimado COP" value={currency(Number(lead.estimated_value_cop ?? 0), "COP")} />
            <Info label="Valor estimado USD" value={currency(Number(lead.estimated_value_usd ?? 0), "USD")} />
          </div>
          {lead.observations && <p className="mt-5 rounded-2xl bg-slate-50 p-4 text-sm text-slate-700">{lead.observations}</p>}
        </Card>
      ...
      <div className="grid gap-6 xl:grid-cols-2">
        <Card>
          ...
          <div className="mt-4 space-y-3">
            ...
            {!tasks?.length && <p className="text-sm text-slate-500">Este lead todavía no tiene tareas.</p>}
          </div>
        </Card>
          ...
          <div className="mt-4 space-y-3">
            {events?.map((event) => (
              <div key={event.id} className="rounded-2xl border border-slate-100 p-4">
                <div className="font-semibold text-slate-950">{event.description}</div>
                <div className="mt-1 text-sm text-slate-500">{formatDateTime(event.created_at)}</div>
              </div>
            ))}
            {!events?.length && <p className="text-sm text-slate-500">Sin eventos registrados.</p>}
          </div>
        </Card>
      </div>
    </div>
